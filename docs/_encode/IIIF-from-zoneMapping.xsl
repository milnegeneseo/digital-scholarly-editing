...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="titleStmt">
        <xsl:element name="{local-name()}" namespace="http://www.tei-c.org/ns/1.0">
            <xsl:comment>STOP! DO NOT EDIT THIS FILE! This file has been processed with XSLT to output IIIF URLs for regions of the page surface. 
            Find the version of the file without the word "processed" in the filename.
            </xsl:comment>
            <xsl:apply-templates/>
        </xsl:element>
    </xsl:template>
